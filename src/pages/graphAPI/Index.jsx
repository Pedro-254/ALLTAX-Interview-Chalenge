...
function Graph() {
  ...
  useEffect(() => {
    if (marcaSelecionada) {
      fetch(`http://localhost:3000/vendas?marca=${marcaSelecionada}`)
        .then((res) => res.json())
        .then((data) => {
          setVendas(data);
        })
  ...
  const dadosGrafico = gerarDadosGrafico(vendas);
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ width: '100%', height: 500 }}>
        <ResponsiveContainer>
          <LineChart data={dadosGrafico} margin={{ top: 20, right: 30, left: 40, bottom: 5 }}>
            <CartesianGrid />
            <XAxis
              label={{ value: 'Meses', position: 'center', dy: 10 }}
              dataKey="mes"
